...
const eventTypes = ['All', 'Art', 'Live Music', 'Entertainment', 'Local Flavor', 'Lifestyle'];
const eventTags = ['All', 'All Ages', 'Cultural', 'Annual', 'Free', 'Outdoor', 'Wellness'];
    ...
    const filteredEvents = useMemo(() => {
        ...
        return events
            ...
            .filter(event => {
                ...
                return activeTags.every(tag => event.tags && event.tags.includes(tag));
            })
            .filter(event => {
                const lowerSearchTerm = searchTerm.toLowerCase();
                return (event.title && event.title.toLowerCase().includes(lowerSearchTerm)) ||
    ...
    return (
        <>
            <Helmet>
                ...
                <meta name="description" content="Find exciting events happening near you in the 785 area." />
            </Helmet>
            <div className="container mx-auto px-4 py-8 md:py-12">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8 p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-slate-200 shadow-md space-y-4">
                    <div className="relative w-full">
                        ...
                        <Input type="text" placeholder="Search by event name or description..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 h-12 text-lg" />
                    </div>
                    <div className="flex flex-wrap gap-2 justify-center border-b pb-4">
                        {eventTypes.map(type => (
                            <Button key={type} variant={activeType === type ? 'default' : 'outline'} onClick={() => setActiveType(type)} className={`transition-all duration-200 ${activeType === type ? 'bg-slate-900 text-white' : 'bg-white'}`}>{type}</Button>
                        ))}
                    </div>
                    <div className="flex flex-wrap gap-2 justify-center">
                        {eventTags.map(tag => (
                            <Button key={tag} variant={(tag === 'All' && activeTags.length === 0) || activeTags.includes(tag) ? 'secondary' : 'ghost'} size="sm" onClick={() => toggleTag(tag)} className={`rounded-full transition-all duration-200 ${(tag === 'All' && activeTags.length === 0) || activeTags.includes(tag) ? 'bg-slate-700 text-white' : 'bg-slate-100'}`}>{tag}</Button>
                        ))}
                    </div>
                </motion.div>
