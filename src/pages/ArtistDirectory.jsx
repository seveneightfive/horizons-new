...
const ArtistDirectory = () => {
  ...
  const filteredArtists = useMemo(() => {
    ...
    const selectedTypeObject = artistTypes.find(
      (t) => t.name === activeTypeFilter,
    );
    ...
    return artists
      ...
      .filter((artist) => {
        if (activeTypeFilter !== "Band" || activeGenreFilter === "All")
          return true;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Discover and follow talented artists."
        />
      </Helmet>
      <div className="container mx-auto px-4 py-8 md:py-12">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-slate-600 max-w-2xl mx-auto">
            Explore, discover, and follow your favorite artists.
          </p>
        </motion.div>
              ...
              >
                ...
                {genres.map((genre) => (
                  <Button
                    key={genre}
                    variant={
                      activeGenreFilter === genre ? "default" : "outline"
                    }
                    size="sm"
          ...
          {filteredArtists.length > 0 ? (
            filteredArtists.map((artist) => (
              <motion.div key={artist.id} variants={itemVariants}>
                <ArtistDirectoryCard
                  artist={artist}
                  upcomingEventsCount={artist.events.length}
                />
              </motion.div>
            ))
          ) : (
